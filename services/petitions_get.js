...
const firstfile = "https://petition.parliament.uk/petitions.json";
...
var processMany = function (petitions, callback) {
  for (var increment in petitions.data) {
    getJSON(
      "https://petition.parliament.uk/petitions/" + petitions.data[increment].id + ".json",
        ...
        if (handleError(error)) {
          ...
          for (var i = 0; i < preCon.length; i++) {
            response.data.attributes.signatures_by_constituency[i].importance =
              response.data.attributes.signatures_by_constituency[i].signature_count /
